...
def time_queue_push(initial_size, n_trials):
    """ Finds average time for pushing onto a queue of a given initial size"""
    s = Queue()
    ...
    template = "Initial Queue size = {:,d} -> avg. time/push for {:,d} enqueues is {:10.8f}"
    print((template.format(initial_size, n_trials, time_per_operation)))
...
def run_tests():
    ...
    time_queue_push(initial_size, n_trials)
